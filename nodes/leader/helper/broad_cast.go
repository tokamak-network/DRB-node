...
func broadCastS(h host.Host, roundNum string, eoaAddress string, secret [32]byte) {
	...
	for _, op := range eth.ActivatedOperators {
		...
		if err != nil {
			logger.Infof("Failed to create stream to peer %s: %v", nodeInfo[op.Hex()].PeerID, err)
			continue
		}
		if err := json.NewEncoder(stream).Encode(message); err != nil {
			logger.Infof("Failed to send Secret to regular node: %v", err)
		} else {
			logger.Infof("Secret sent to regular node %v for round %s", op, roundNum)
		}
		stream.Close()
	}
}
...
func BroadCastCOS(h host.Host, roundNum string, eoaAddress common.Address, cos [32]byte) {
	...
	for _, op := range eth.ActivatedOperators {
		...
		if err := json.NewEncoder(stream).Encode(message); err != nil {
			logger.Infof("Failed to send CO to regular node: %v", err)
		} else {
			logger.Infof("CO sent to regular node %v for round %s", op, roundNum)
		}
		stream.Close()
	}
}
...
func BroadCastCVS(h host.Host, roundNum string, eoaAddress common.Address, cvs [32]byte) {
	...
	for _, op := range eth.ActivatedOperators {
		...
		if err := json.NewEncoder(stream).Encode(message); err != nil {
			logger.Infof("Failed to send CVS to regular node: %v", err)
		} else {
			logger.Infof("CVS sent to regular node %v for round %s", op, roundNum)
		}
		stream.Close()
	}
}
