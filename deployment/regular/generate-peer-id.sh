#!/bin/bash

# Define the file path for the Go source 
GO_FILE_PATH="../../cmd/regulargenerator/main.go"
# Define the name for the compiled binary
BINARY_NAME="regular_peer_id_generator"
# Define the target file to be checked/created
TARGET_FILE="static-key/regularnode.bin"

echo "Generating regular node peer ID for production from $GO_FILE_PATH..."

# Create static-key directory if it doesn't exist
mkdir -p static-key

# Check if the target file already exists
if [ -f "$TARGET_FILE" ]; then
    echo "File '$TARGET_FILE' already exists. The script will load existing peer ID."
    echo "To generate a new peer ID, please delete the existing file first."
fi

# Build the Go program
echo "Building the Go program..."
go build -o "$BINARY_NAME" "$GO_FILE_PATH"

# Check if the build was successful
    if [ $? -eq 0 ]; then
        echo "Build successful. Executing the program..."
        chmod +x "$BINARY_NAME"
        ./"$BINARY_NAME" 0
        
        # Clean up binary
        rm -f "$BINARY_NAME"
else
    echo "Go build failed. Exiting."
    exit 1
fi
