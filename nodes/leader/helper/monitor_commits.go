...
func checkRoundsForCompletion(fallbackEthClient *fallback_ethclient.FallbackRPCClient) {
	...
	for round := range eoasForRounds {
		...
		if err != nil {
			logger.Infof("Failed to load leader commits: %v", err)
			continue
		}
		...
		if !isMerkleRootSubmitted(leaderCommits, round) {
			logger.Infof("Merkle root not submitted for round %s. Skipping random number generation.", round)
			continue
		}
		...
		for i, operator := range operatorAddresses {
			...
			if !exists || commitData.SecretValue == [32]byte{} {
				logger.Infof("EOA %s has not submitted a secret value for round %s.", operator.Hex(), round)
			...
			if len(commitData.Sign["v"]) == 0 || len(commitData.Sign["r"]) == 0 || len(commitData.Sign["s"]) == 0 {
				logger.Infof("Incomplete signature for EOA %s in round %s", operator.Hex(), round)
				allEOAsSubmitted = false
				continue
			}
			...
			if err != nil {
				logger.Infof("Error parsing v value for EOA %s in round %s: %v", operator.Hex(), round, err)
				allEOAsSubmitted = false
				continue
			}
		...
		if allEOAsSubmitted {
			logger.Infof("All EOAs have submitted for round %s. Initiating random number generation.", round)
			var err error
			if !CvOnChain {
			...
			if err != nil {
				logger.Infof("Failed to execute random number generation transaction for round %s: %v", round, err)
			} else {
...
func LoadNodeData(round string) ([][]byte, [][]byte, [][]byte, []uint8, []common.Hash, []common.Hash) {
	...
	for i, operator := range operatorAddresses {
		...
		if len(commitData.Sign["v"]) == 0 {
			logger.Infof("Empty 'v' value for EOA %s in round %s", operator.Hex(), round)
			vs = append(vs, 0)
		} else {
			vStr := commitData.Sign["v"]
			vValue, err := strconv.ParseUint(vStr, 10, 8)
		...
		if len(commitData.Sign["r"]) == 0 {
			logger.Infof("Empty 'r' value for EOA %s in round %s", operator.Hex(), round)
		...
		if len(commitData.Sign["s"]) == 0 {
			logger.Infof("Empty 's' value for EOA %s in round %s", operator.Hex(), round)
...
func generateRandomNumberTransaction(fallbackEthClient *fallback_ethclient.FallbackRPCClient, round string, secrets [][]byte, vs []uint8, rs []common.Hash, ss []common.Hash) error {
	logger.Infof("Preparing to execute generateRandomNumber...")
	...
	if !exists {
		logger.Infof("No reveal order found for round %s.", round)
		return nil
	}
	...
	logger.Infof("Transaction submitted. TX Hash: %s", tx.Hash().Hex())
	return nil
}
...
func generateRandomNumberTransactionSomeCvOnChain(fallbackEthClient *fallback_ethclient.FallbackRPCClient, round string, secrets [][]byte, vs []uint8, rs []common.Hash, ss []common.Hash) error {
	logger.Infof("Preparing to execute generateRandomNumberTransactionSomeCvOnChain...")
...
func markRoundCompleted(leaderCommits map[string]utils.LeaderCommitData, round string) {
	...
	if err != nil {
		logger.Infof("Failed to save updated leader commits: %v", err)
	}
