	...
	for {
		...
		case vLog := <-logs:
			...
			case CvsEventSig:
				...
				if err != nil {
					logger.Infof("Failed to decode CvSubmitted event log: %v", err)
					continue
				}
			...
			case CoSubmittedSig:
				...
				if err != nil {
					logger.Infof("Failed to decode CoSubmitted event log: %v", err)
					continue
				}
...
func updateCOS(fallbackEthClient *fallback_ethclient.FallbackRPCClient, round string, eoa common.Address, cos [32]byte) {
	...
	if AllCosReceivedUnlocked(round) {
		logger.Infof("All COS received for round %s.", round)
		err := commitreveal2.DetermineRevealOrder(round, eth.ActivatedOperators)
		if err != nil {
			logger.Infof("Failed to determine reveal order for round %s: %v", round, err)
			return
		}
