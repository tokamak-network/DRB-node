...
func LoadLeaderCommitData(roundNum, eoaAddress string) (*LeaderCommitData, error) {
	...
	logger.Infof("Loading commit data for key: %s", key) // Debug log for the key
	...
	logger.Infof("Loaded commit data for key: %s, CVS: %v", key, commitData.Cvs) // Debug log for loaded data
...
func SaveLeaderCommitData(commitData LeaderCommitData) error {
	...
	logger.Infof("Saved commit data for key: %s", key) // Debug log for commit save
	return nil
}
