...
func RegisterNode(s network.Stream, filePath, abiFilePath string) error {
	...
	logger.Infof("Verified registration for PeerID: %s", req.PeerID)
	...
	logger.Infof("Successfully registered or updated EOA %s with NodeInfo: IP=%s, Port=%s, PeerID=%s.", req.EOAAddress, ip, port, req.PeerID)
