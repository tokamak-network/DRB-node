...
func LoadContractABI(filename string) (abi.ABI, error) {
	...
	logger.Log.Infof("Successfully loaded contract ABI from %s", filename)
...
func readFile(filename string) ([]byte, error) {
	...
	if err != nil {
		logger.Log.Errorf("Failed to read ABI file: %v", err)
...
func parseABIJSON(fileContent []byte) (struct {
	...
	if err := json.Unmarshal(fileContent, &abiObject); err != nil {
		logger.Log.Errorf("Failed to parse ABI JSON: %v", err)
...
func convertToABI(abiObject struct {
	...
	if err != nil {
		logger.Log.Errorf("Failed to parse contract ABI: %v", err)
