...
func DetermineRevealOrder(roundNum string, activatedOperators []common.Address) error {
	...
	if _, exists := data[roundNum]; exists {
		logger.Infof("Reveal order already exists for round %s. Skipping calculation.", roundNum)
		return nil
	}

	logger.Infof("Determining reveal order for round %s...", roundNum)

	operators := eth.ActivatedOperators

	if len(operators) == 0 {
		logger.Infof("No activated operators found for round %s", roundNum)
	...
	for _, eoaAddress := range operators {
		...
		if err != nil {
			logger.Infof("Failed to load COS for operator %s in round %s: %v", eoaAddressStr, roundNum, err)
			return fmt.Errorf("failed to load COS for operator %s", eoaAddressStr)
		}

		if commitData.Cos == [32]byte{} {
			logger.Infof("Missing COS for operator %s in round %s", eoaAddressStr, roundNum)
	...
	if err != nil {
		logger.Infof("Failed to save reveal order for round %s: %v", roundNum, err)
		return fmt.Errorf("failed to save reveal order for round %s", roundNum)
	}

	logger.Infof("Reveal order determined and stored for round %s", roundNum)
	return nil
}
...
func DetermineRevealOrderForRegular(roundNum string, activatedOperators []common.Address, filePath string) (bool, error) {
	...
	if _, exists := data[roundNum]; exists {
		logger.Infof("Regular Reveal order already exists for round %s. Skipping calculation.", roundNum)
		return false, nil
	}
